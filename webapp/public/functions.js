...
function logedUserID(){//get userID
    document.getElementById("userLoggedIn").textContent = "Logged In: " + (int(localStorage.getItem("userID")) == -1 ? "Guest" : localStorage.getItem("userID"));
...
async function getMaxOrderId() {
    try {
        const response = await fetch('http://localhost:3000/maxOrderID', {
            method: 'GET'
        });
...
async function orderPush(){
    ...
    orderId=parseInt(await getMaxOrderId())+1;
    timeStamp = Date.now();
